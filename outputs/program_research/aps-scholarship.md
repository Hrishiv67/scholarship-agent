# American Physical Society Scholarship

**URL:** https://www.aps.org/programs/honors/scholarships/
**Application Opens:** Not found on page -- typically November
**Deadline:** Not found -- check https://www.aps.org/programs/honors/scholarships/
**Award:** $2,000-$3,000 per year
**Tier:** Competitive
**Last Verified:** 2026-08-25
**Research Confidence:** none

## Eligibility
Could not determine -- check program website

## Requirements
- application
- essay
- transcript
- teacher_rec

## Essay Prompts (2026-2027 cycle)
Not listed on official page -- check the application portal directly.

## What a Strong Application Looks Like
See program website for selection criteria.

## Recommended Prep Timeline
Begin preparation 2-3 months before the February deadline.

## Notes
Research failed: API call failed: Error code: 404 - {'type': 'error', 'error': {'type': 'not_found_error', 'message': 'model: claude-sonnet-4-5-20251015'}, 'request_id': 'req_011CeNiHix5SXGGAkuPWT2TH'}. Check https://www.aps.org/programs/honors/scholarships/ manually.

---
*Researched by `calendar_agent` on 2026-08-25. Re-run `python -m calendar_agent.research` to refresh.*
